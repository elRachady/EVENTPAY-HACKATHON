...
class LNbitsService {
  ...
  async createInvoice(amount, memo = '', expiry) {
    try {
      console.log('DEBUG amount:', amount, typeof amount); // Ajoute ceci

      // Validation stricte du montant
      const parsedAmount = Number(amount);
      if (!parsedAmount || !Number.isInteger(parsedAmount) || parsedAmount <= 0) {
        throw new Error('Le montant de la facture doit être un entier positif');
      }
      ...
      // Ajoute expiry SEULEMENT si c'est un entier strictement positif
      if (
        expiry !== undefined &&
        expiry !== null &&
        expiry !== "" &&
